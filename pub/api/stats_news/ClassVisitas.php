...
class ClassVisitas extends ClassConexao{
    ...
    {
        $Select=$this->Conectar()->prepare("select * from visitas_news where Ip=:ip and Data=:datas order by Id desc");
        ...
        if($Select->rowCount() == 0){
            ...
            $Hora_1=date("H:i");
                ...
                $Select=$this->Conectar()->prepare("update  visitas_news set hits=hits+1,Hora = '".$Hora_1."' where Id=".$Id_actual);
                $Select->execute();
    ...
    {
        $Select=$this->Conectar()->prepare("insert into visitas_news values (:id , :ip , :datas , :hora, :hits)");
