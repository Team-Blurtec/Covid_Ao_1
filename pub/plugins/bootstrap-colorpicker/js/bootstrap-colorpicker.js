...
var HSVAColor = function () {
  ...
  _createClass(HSVAColor, [{
    ...
    value: function toString() {
      return this.h + ', ' + this.s + '%, ' + this.v + '%, ' + this.index2;
    }
  }]);
  ...
  _createClass(ColorItem, [{
    ...
    value: function parse(color) {
      ...
      if (color instanceof HSVAColor) {
        color = [color.h, color.s, color.v, isNaN(color.index2) ? 1 : color.index2];
      } else {
  ...
  _createClass(PickerHandler, [{
    ...
    value: function update() {
      ...
      if (alphaGuide.length) {
        alphaGuide.css(vertical ? 'top' : 'left', (vertical ? slider.alpha.maxTop : slider.alpha.maxLeft) * (1 - hsva.index2));
      }
